...
- 🐛 **Report a bug** - open a [bug report](https://github.com/NotAFlightRisk/ai-usage-dashboard/issues/new?template=bug_report.yml)
- 💡 **Suggest a feature** - open a [feature request](https://github.com/NotAFlightRisk/ai-usage-dashboard/issues/new?template=feature_request.yml)
...
5. **Run `npm run check`, `npm run format` and `npm test`** before you push.
...
The most useful contribution is a reader for a tool we don't cover yet. One file in
`src/lib/server/sources/`, exporting an id, the directory it reads, the file extension, and a
`parse` that turns file contents into events. `codex.ts` is the fiddly one to copy from, since it
has to dedupe. Add it to `sources/index.ts`, and a test with a real (redacted) sample.

If you can't work out where a tool keeps its token counts, open an issue with a sample file
anyway - that's the hard half.
